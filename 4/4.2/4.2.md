...
| **Cod Escenario** | **Atributo de Calidad** | **Estímulo**                             | **Fuente del Estímulo** | **Artefacto**                    | **Entorno**                         | **Respuesta**                                                | **Medida de Respuesta**                                      |
|------------------|-------------------------|------------------------------------------|-------------------------|----------------------------------|-------------------------------------|-------------------------------------------------------------|--------------------------------------------------------------|
| REC-M01           | Mantenibilidad          | Solicitud de cambio en los criterios de recomendación | Equipo de desarrollo     | Módulo de Recomendaciones        | Operación normal                    | El sistema permite ajustar los criterios sin afectar otras partes del módulo  | Tiempo de implementación del cambio menor a 4 horas          |

### Táctica Elegida

**Modularización de Funcionalidades**: Para este escenario, se selecciona la táctica de **Modularización de Funcionalidades**, donde las reglas de recomendación y el procesamiento se separan en submódulos independientes, permitiendo realizar cambios en una parte sin afectar otras.

### Documentación de la Decisión (ADR)

**Título**:  
Elección entre modularización vs módulo monolítico para el módulo de recomendaciones.

**Contexto**:  
El módulo de recomendaciones maneja varias funcionalidades, como el análisis de preferencias de usuario, historial de navegación y condiciones climáticas en tiempo real. Para mejorar la mantenibilidad, se propone separar estos componentes en submódulos, facilitando los futuros cambios en los criterios de recomendación sin impactar otras áreas del sistema.

**Alternativas**:
1. **Modularización de funcionalidades**  
   - Dividir las funcionalidades de análisis de preferencias, ubicación y clima en módulos independientes.
   - Facilita la modificación de los criterios de recomendación sin riesgo de impactar otras partes del sistema.
   - Aumenta la mantenibilidad a largo plazo y mejora la facilidad de prueba y depuración.

2. **Mantener un módulo monolítico**  
   - Mantener todas las funcionalidades dentro de un solo módulo.
   - Menor esfuerzo inicial, pero podría complicar la implementación de futuros cambios.
   - Aumenta el riesgo de introducir errores que afecten otras partes del sistema.

**Criterios de elección**:  
- Minimizar el impacto de los cambios en otras funcionalidades.
- Facilitar el mantenimiento y la implementación de nuevas características.
- Aumentar la flexibilidad del sistema para adaptarse a futuras mejoras.

**Decisión**:  
Se elige la **Modularización de funcionalidades**.

**Sustento**:  
Dividir el módulo de recomendaciones en submódulos facilita la implementación de nuevos criterios y ajustes en las recomendaciones sin riesgo de afectar otras funcionalidades del sistema. Aunque esto implica una inversión inicial, mejora significativamente la mantenibilidad y reduce el riesgo de introducir errores.
...
| **Cod Escenario** | **Atributo de Calidad** | **Estímulo**                             | **Fuente del Estímulo** | **Artefacto**                    | **Entorno**                         | **Respuesta**                                                | **Medida de Respuesta**                                      |
|------------------|-------------------------|------------------------------------------|-------------------------|----------------------------------|-------------------------------------|-------------------------------------------------------------|--------------------------------------------------------------|
| REC-M02           | Mantenibilidad          | Necesidad de agregar nuevas categorías de recomendaciones | Equipo de desarrollo     | Módulo de Recomendaciones        | Operación normal                    | El sistema permite agregar nuevas categorías sin afectar la lógica existente | Implementación del cambio en menos de 6 horas                |

### Táctica Elegida

**Disminuir acoplamiento**: En este escenario, se selecciona la táctica de **Disminuir Acoplamiento**, lo que implica encapsular las funcionalidades de categorización para que puedan ser modificadas o extendidas sin impactar otras áreas del módulo de recomendaciones.

### Documentación de la Decisión (ADR)

**Título**:  
Elección entre disminuir acoplamiento vs mantener acoplamiento para el módulo de recomendaciones.

**Contexto**:  
El equipo de desarrollo necesita agregar nuevas categorías de recomendaciones (por ejemplo, deportes, cultura). Si las funcionalidades de categorización están acopladas con otras áreas del sistema, los cambios podrían afectar otros componentes, complicando el mantenimiento y aumentando el riesgo de errores.

**Alternativas**:
1. **Disminuir acoplamiento**  
   - Encapsular las funcionalidades de categorización en un componente independiente.
   - Facilita la extensión y actualización de las categorías sin impactar otras partes del sistema.
   - Mejora la mantenibilidad y reduce el riesgo de introducir errores al modificar o agregar categorías.

2. **Mantener acoplamiento**  
   - Dejar las funcionalidades de categorización acopladas con otras partes del módulo.
   - Aumenta el riesgo de que los cambios en la categorización afecten otras funcionalidades del sistema.
   - Dificulta la implementación de nuevos cambios y pruebas.

**Criterios de elección**:  
- Minimizar el impacto de los cambios en la lógica de categorización sobre otras funcionalidades.
- Facilitar la extensión y actualización de categorías.
- Asegurar que las modificaciones puedan realizarse eficientemente.

**Decisión**:  
Se elige **Disminuir Acoplamiento**.

**Sustento**:  
Al desacoplar las funcionalidades de categorización, se mejora la capacidad del sistema para manejar cambios y adiciones sin comprometer otras áreas del módulo. Esto asegura una mayor flexibilidad en la implementación de nuevas categorías y mejora la mantenibilidad del sistema a largo plazo.
